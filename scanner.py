...
def process_symbol(symbol: str, sentiment: str = "NEUTRAL"):
    """
    ...
    result = analyze_data(df_15m, df_htf=df_1h, sentiment=sentiment)
...
# Professional Asset Whitelist (Top-tier liquidity and volume)
PROFESSIONAL_WHITELIST = [
    'BTC/USDT:USDT', 'ETH/USDT:USDT', 'SOL/USDT:USDT', 'XRP/USDT:USDT',
    'BNB/USDT:USDT', 'ADA/USDT:USDT', 'AVAX/USDT:USDT', 'DOT/USDT:USDT',
    'LINK/USDT:USDT', 'POL/USDT:USDT', 'PAXG/USDT:USDT', 'BCH/USDT:USDT'
]

def scan_markets(cooldown_tracker: dict):
    """
    Scans the curated list of professional USDT perpetual futures markets in parallel.
    Uses a cooldown_tracker (dict) to prevent duplicate signals within a timeframe.
    """
    from news import get_market_sentiment
    print("Checking global market sentiment from news feeds...")
    market_sentiment = get_market_sentiment()
    print(f"Current Market Sentiment: {market_sentiment}")

    symbols_to_scan = PROFESSIONAL_WHITELIST
    print(f"Starting parallel scan for {len(symbols_to_scan)} professional pairs...")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_symbol = {executor.submit(process_symbol, symbol, market_sentiment): symbol for symbol in symbols_to_scan}
